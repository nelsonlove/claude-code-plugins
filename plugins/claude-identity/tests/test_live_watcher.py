...
from lib.live_note import write_live_note
...
def _note_path(live_notes_dir, handle):
    return live_notes_dir / f"{handle}.md"
...
def test_detect_no_change_after_self_write(tmp_home, tmp_path):
    ...
    result = detect_user_edit(tmp_home, SID, "quill", live_notes_dir=tmp_path)
...
def test_modified_field_rewrite_alone_does_not_trigger(tmp_home, tmp_path):
    ...
    text = path.read_text()
    # Surgically rewrite ONLY the frontmatter's `modified:` line.
...
def test_detect_user_edit_when_body_changes(tmp_home, tmp_path):
    ...
    path = _note_path(tmp_path, "quill")
    text = path.read_text()
    path.write_text(text.replace("hello", "Nelson's edit here"))
    result = detect_user_edit(tmp_home, SID, "quill", live_notes_dir=tmp_path)
...
def test_accept_change_silences_subsequent_polls(tmp_home, tmp_path):
    ...
    result2 = detect_user_edit(tmp_home, SID, "quill", live_notes_dir=tmp_path)
